...
namespace DiFfRG
{
  ...
  class Zc_kernel
  {
    ...
    // clang-format on
    {
      ...
      double fenv[(DiFfRG::zc_qcd::nenv) > 0 ? (DiFfRG::zc_qcd::nenv) : 1];
      DiFfRG::zc_qcd::fill(fenv, l1, cos1, p);
      ...
      return fma(-0.125, _cse1 * _den3 * _den4 * DiFfRG::zc_qcd::tr0(fenv) * powr<2>(_interp5) * (_interp2 * nthk1 + _interp1 * (nthk2 + 50. * (-nthk1 + nthk3))), fma(0.125, _cse1 * _den2 * _den5 * DiFfRG::zc_qcd::tr1(fenv) * powr<2>(_interp5) * (_interp2 * nthk0 + _interp1 * (nthk4 + 50. * _den1 * powr<6>(k) * (-nthk0 + nthk5))), 0.));
    }
