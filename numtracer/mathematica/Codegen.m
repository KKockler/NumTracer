...
cppFlat[e_] :=
   StringReplace[FunKit`CppForm[e], {"\n" -> " ", "\r" -> " ", "\t" -> " "}];
...
scaleStrInv[str_, s_] :=
   "sc<numtracer::Lit<numtracer::Cx{" <> cppNum[s] <> ", 0.0}>>(" <> str <> ")";

wrapContractInv[{one_}] :=
   one;

wrapContractInv[many_] :=
   "contract(" <> StringRiffle[many, ", "] <> ")";
...
compileTInv[e_, ids_, env_, mask_, nc_] :=
   With[{h = Hash[{e, ids, env, mask, nc}]},
      Lookup[$ctCache, h, $ctCache[h] = compileTInvBody[e, ids, env, mask, nc]]
   ];

compileTInvBody[e_, ids_, env_, mask_, nc_] :=
   Which[
      tensorQ[e],
...
colourEntangledQ[e_] :=
   !FreeQ[e, Alternatives @@ ctHeadsInv];
...
mergeColNet["SUNNet{}", b_] :=
   b;

mergeColNet[a_, "SUNNet{}"] :=
   a;
...
diagDrId[name_, scale_] :=
   Module[{key = {name, scale}},
...
diagComp2Dr[spec_, scale_, dim_] :=
   Module[{rules = Flatten[{spec}], named, def},
...
ntCompileJobs[] :=
   With[{e = Environment["NT_GEN_JOBS"], avail = ntAvailMemGB[]},
      Which[
...
resolveGenCxx[] :=
   Module[{env = Environment["NT_GEN_CXX"], path, comps, clangPick, pick, name, inst, exe, full},
...
chunkLorInv[lorExpr_, ids_, env_, mask_, nc_] :=
   Which[
      lorExpr === 0,
         {}
      ,
...
chunkLorProd[keepLor_, inner_, ids_, env_, mask_, nc_] :=
   Which[
      inner === 0 || inner === 0.,
         {}
      ,
...
ntSigmaLeg[leg_, termFactors_] :=
   Module[{mu = First[leg], momFactors, terms},
...
ntSigmaTermInfo[term_] :=
   Module[{tf, gs, g1, g2, first, second, din, dout},
      tf =
         If[Head[term] === Times,
            List @@ term
            ,
...
foldDiracSigma[factors_List] :=
   Module[{commPlus, recognized},
      If[!$ntSigmaFold,
         Return[factors]
      ];
...
diracIn[ntGamma[_, a_, _]] :=
   a; diracOut[ntGamma[_, _, b_]] :=
   b;

diracIn[ntGamma5[a_, _]] :=
   a; diracOut[ntGamma5[_, b_]] :=
   b;

diracIn[ntSigma[_, _, a_, _]] :=
   a; diracOut[ntSigma[_, _, _, b_]] :=
   b;

diracIn[ntDeltaDirac[a_, _]] :=
   a; diracOut[ntDeltaDirac[_, b_]] :=
   b;

diracIn[ntDressedNum[_, a_, _]] :=
...
orderDiracChain[facs_] :=
   Module[{byIn, cur, toks = {}, seen = {}},
...
gammaTraceSum[lst_] :=
   canonGammaTrace[Length[lst]] /. Dispatch[Thread[Range[Length[lst]] -> lst]];
...
diracTrace[toks_] :=
   Module[{n5 = Count[toks, $g5], gammas = DeleteCases[toks, $g5], sign},
...
expandDiracComponent[factors_List] :=
   Module[{dir, rest},
...
dressedSlotStr[gf : ntDressedNum[_, _, _], env_] :=
   With[{h = Hash[{gf, env}]},
...
compileDirac[factors_, ids_, env_, mask_, nc_] :=
   Module[
...
(* an ntSigma leg → its C++ arg string. A slashed leg is a list of {coeff, q} pairs (q a literal
   ...
   if a leg momentum is somehow not in env (it always should be — momentumOf collected it). *)
      ...
      sigStr[legA_, legB_] :=
         Module[{ta = legA[[1]], tb = legB[[1]], sa = legStr[legA], sb = legStr[legB]},
            Which[
               ta === "free" && tb === "free",
                  "dcomm(" <> sa <> ", " <> sb <> ")"
               ,
...
unitLoopFrameSpec[frame_, pSym_, magSym_] :=
   Module[{defs = <||>, groups = {}, n = 0, nf},
      nf =
         Association @
            KeyValueMap[
               Function[{q, comps},
...
polyFrameSpec[frame_] :=
   Module[{defs = <||>, vals, rules = {}, polyFrame},
...
unitLoopOkQ[frame_, pSym_, magSym_] :=
   AllTrue[
      Values[frame]
      ,
      Function[comps,
...
numericComponents[env_, frame_, symDefs_, unitGroups_ : {}] :=
   Module[
...
(* Emit through the generator's `env` (a LorentzEnv bound to nsym) — the sole construction path for
   MPoly now that the bare-nsym factories are private. *)
      mpcpp[e_] :=
         Module[{rules = CoefficientRules[e, usyms]},
            If[rules === {},
               "env.zero()"
               ,
...
(* The lc<k>()/dc<k>() net-level CSE runs for BOTH paths: the dressed (collected) lnet builders are
   ...
   which left the dressed generator C++ bloated and its -O0 compile ~2x slower. *)
      Module[{lTerms = Flatten[lStr], dTerms = Flatten[dStr], lCount, dCount, lMap = <||>, dMap = <||>, li = 0, di = 0, lRef, dRef},
         With[{
            ntT =
               First @
                  AbsoluteTiming[
         ...
         lRef[t_] :=
            If[KeyExistsQ[lMap, t],
               lMap[t] <> "()"
               ,
               t
            ];
         dRef[t_] :=
            If[KeyExistsQ[dMap, t],
               dMap[t] <> "()"
               ,
               t
            ];
         ...
         With[{
            ntT =
               First @
                  AbsoluteTiming[
                     dStr = Map[dRef, dStr, {2}];
                     lStr = Map[lRef, lStr, {2}];
                  ]
         },
...
(* units is emitted BEFORE the LorentzEnv because the env binds both nsym and the unit groups;
   comp/atomDen and the trace entry points are then built through `env`. *)
                  "  std::vector<std::vector<int>> units = {" <> StringRiffle[("{" <> StringRiffle[bb /@ #, ","] <> "}")& /@ unitG, ","] <> "};\n"
                  ,
                              ...
                              With[{uSca = DeleteDuplicates[scaIdxRows]},
                                 With[{scaPos = AssociationThread[uSca -> Range[Length[uSca]] - 1]},
                                    StringJoin[
                                       ...
                                       ,
                                       "  std::vector<std::vector<int>> sidxU = {" <> StringRiffle[("{" <> StringRiffle[bb /@ #, ","] <> "}")& /@ uIdx, ","] <> "};\n"
                                       ,
                                       ...
                                       ,
                                       "  std::vector<std::vector<int>> dscU = {" <> StringRiffle[("{" <> StringRiffle[bb /@ #, ","] <> "}")& /@ uSca, ","] <> "};\n"
                                       ,
...
(* colnets HASH-CONSED for the same reason (0.57 MB -> 0.115 MB on the Fierz gate: 3470 rows,
   ...
   nets — 4.8x fewer on that flow — which is a RUN saving on top of the compile one. *)
                  ...
                  ,
                  "  std::vector<std::vector<int>> groups = {" <> StringRiffle[("{" <> StringRiffle[bb /@ #, ","] <> "}")& /@ groups, ","] <> "};\n"
                  ,
                  "  GlobalEnv g;\n"
                  ,
...
ntConstFn[constExpr_, decor_, constParams_, sns_] :=
...
ntWrapBody[kns_, classStr_, name_] :=
   If[kns === None || kns === "",
      {classStr}
      ,
...
ntRuntimeIncludes[runInc_] :=
   If[runInc === None || runInc === "",
      {}
      ,
      {runInc}
   ];
...
ntDressType[dressTy_] :=
   If[dressTy === Automatic,
      "auto"
      ,
      dressTy
   ];
...
resolveGenLib[incDir_] :=
   Module[{env, base, cands, lib},
