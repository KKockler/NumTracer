...
static inline auto probe_full(const double& l1, const double& cos1, const double& cos2, const double& phi, const double& p, const double& k)
{
  ...
  DiFfRG::zaaqbq1_small_ref::fill(fenv, l1, cos1, cos2, phi, p, dr_0, dr_1);
...
static inline auto probe_proj(const double& l1, const double& cos1, const double& cos2, const double& phi, const double& p, const double& k)
{
  ...
  const auto _interp13 = ntStub(29441., 1. * sqrt(powr<2>(l1) - 2. * cosl1p1 * l1 * p + powr<2>(p)));
  const auto _interp14 = ntStub(89189., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) - 2. * cosl1p1 * l1 * p + powr<2>(p)));
  const auto _interp15 = ntStub(62323., 0.816496580927726 * sqrt(powr<2>(l1) + cosl1p2 * l1 * p + powr<2>(p)));
  const auto _interp16 = ntStub(29441., 1. * sqrt(powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p)));
  const auto _interp17 = ntStub(89189., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p)));
  const auto _interp18 = ntStub(98454., 0.7071067811865475 * sqrt(powr<2>(l1) + (-cosl1p1 + cosl1p2) * l1 * p + 2. * powr<2>(p)));
  const auto _den1 = powr<-1>(1. + powr<6>(k));
  const auto _den2 = powr<-2>(_interp3 * _interp4 + _interp2 * powr<2>(l1));
  const auto _den3 = powr<-1>(_interp14 * _interp3 + _interp13 * (powr<2>(l1) - 2. * cosl1p1 * l1 * p + powr<2>(p)));
  const auto _den4 = powr<-1>(_interp17 * _interp3 + _interp16 * (powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p)));
  const auto _den5 = 3. * powr<-1>(3. * _interp3 * _interp9 + _interp8 * (3. * powr<2>(l1) - 6. * (cosl1p1 + cosl1p2) * l1 * p + 4. * powr<2>(p)));
  const auto _cse1 = powr<-2>(p);
  const auto _cse2 = _interp3 * _interp5;
  ...
  return -0.0003662109375 * fma(_cse1, _cse8 * _den2 * _den5 * DiFfRG::zaaqbq1_small_ref::tr2(fenv) * _interp1 * _interp10, fma(2., _cse1 * _cse8 * _den2 * _den3 * _den5 * DiFfRG::zaaqbq1_small_ref::tr1(fenv) * _interp10 * _interp11 * _interp12, fma(_cse1, _cse8 * _den2 * _den3 * _den4 * DiFfRG::zaaqbq1_small_ref::tr0(fenv) * _interp11 * _interp15 * _interp18, 0.)));
}
