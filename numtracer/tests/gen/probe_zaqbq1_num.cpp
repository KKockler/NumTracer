...
static inline auto probe_full(const double& l1, const double& cos1, const double& cos2, const double& p, const double& k)
{
  ...
  const double dr_4 = ntStub(30415., 1. * sqrt(powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p)));
  const double dr_5 = -sqrt(powr<-1>(powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p))) * ntStub(80376., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p))) * ntStub(85453., 1. * k) - ntStub(85453., 1. * sqrt(powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p)));
  ...
  DiFfRG::zaqbq1_num::fill(fenv, l1, cos1, cos2, p, dr_0, dr_1, dr_2, dr_3, dr_4, dr_5, dr_6, dr_7, dr_8, dr_9);
...
static inline auto probe_proj(const double& l1, const double& cos1, const double& cos2, const double& p, const double& k)
{
  ...
  const auto _den2 = powr<-2>(_interp12 * _interp13 + _interp11 * powr<2>(l1));
  const auto _den3 = powr<-1>(_interp12 * _interp20 + _interp19 * (powr<2>(l1) - 2. * cosl1p1 * l1 * p + powr<2>(p)));
  const auto _den4 = powr<-1>(powr<2>(_interp4) + powr<2>(_interp5 * _interp6 + _interp7 * sqrt(powr<2>(l1) - 2. * cosl1p2 * l1 * p + powr<2>(p))));
  const auto _den5 = powr<-1>(powr<2>(_interp23) + powr<2>(_interp24 * _interp6 + _interp25 * sqrt(powr<2>(l1) + 2. * cosl1p2 * l1 * p + powr<2>(p))));
  const auto _den6 = powr<-1>(powr<2>(_interp8) + powr<2>(_interp10) * (powr<2>(l1) - 2. * (cosl1p1 + cosl1p2) * l1 * p + powr<2>(p)) + _interp6 * _interp9 * (_interp6 * _interp9 + 2. * _interp10 * sqrt(powr<2>(l1) - 2. * (cosl1p1 + cosl1p2) * l1 * p + powr<2>(p))));
  const auto _cse1 = -_interp12;
  const auto _cse2 = _cse1 + _interp16;
  const auto _cse3 = 50. * _cse2 * _den1 * powr<6>(k);
  const auto _cse4 = _cse3 + _interp15;
  const auto _cse5 = _cse4 * _interp13;
  const auto _cse6 = _interp12 * _interp14;
  const auto _cse7 = _cse5 + _cse6;
  return 0.02083333333333333 * fma(-1., _cse7 * _den2 * _den3 * _den6 * DiFfRG::zaqbq1_num::tr4(fenv) * _interp17 * _interp18 * _interp2, fma(-1., _cse7 * _den2 * _den3 * _den5 * DiFfRG::zaqbq1_num::tr3(fenv) * _interp18 * _interp21 * _interp22, fma(_cse7, _den2 * _den4 * _den6 * DiFfRG::zaqbq1_num::tr5(fenv) * _interp1 * _interp2 * _interp3, 0.)));
}
