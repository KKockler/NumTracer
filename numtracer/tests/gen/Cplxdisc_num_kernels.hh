// GENERATED by gen_cplxdisc_num.cpp — do not edit.
#pragma once
#include <cmath>
#include <complex>
namespace numtracer_kernels { namespace cplxdisc_num {
#ifndef NT_TRACE_COMPLEX
#define NT_TRACE_COMPLEX std::complex<double>
#endif
using nt_complex_t = NT_TRACE_COMPLEX;
template<int N> static inline double powr(double x){ double r=1.0; for(int i=0;i<N;++i) r*=x; return r; }
// fundamental-symbol env layout (fill f[i] per call):
//   f[0] = var(0)
//   f[1] = var(1)
//   f[2] = var(4)
//   f[3] = var(2)
static inline constexpr int nenv = 4;
static inline void fill(double *f, [[maybe_unused]] double l1, [[maybe_unused]] double cos1, [[maybe_unused]] double cos2, [[maybe_unused]] double p) {
  f[0] = l1;
  f[1] = cos1;
  f[2] = p;
  f[3] = sqrt(1. - powr<2>(cos1)) * cos2;
}
static inline nt_complex_t tr0([[maybe_unused]] const double *f) {
  const double s0 = f[2];
  const double s1 = f[1];
  const double s2 = f[0];
  const double s4 = s0*(24);
  const double s5 = s1*s4;
  const double s6 = s2*s5;
  return nt_complex_t{0.0, s6};
}
static inline nt_complex_t tr1([[maybe_unused]] const double *f) {
  return nt_complex_t{0.0, (6)};
}
static inline nt_complex_t tr2(const double *f) { return tr1(f); }
static inline double tr3([[maybe_unused]] const double *f) {
  const double s1 = f[2];
  const double s2 = s1*s1;
  const double s3 = (-2)*s2;
  return s3;
}
static inline double tr4([[maybe_unused]] const double *f) {
  const double s1 = f[2];
  const double s2 = (4)*s1;
  const double s3 = f[1];
  const double s4 = s2*s3;
  const double s5 = f[0];
  const double s6 = s4*s5;
  return s6;
}
static inline double tr5(const double *f) { return tr4(f); }
static inline double tr6(const double *f) { return tr3(f); }
static inline double tr7([[maybe_unused]] const double *f) {
  const double s2 = f[1];
  const double s4 = f[3];
  const double s5 = (-0.75)*s4;
  const double s6 = fma((-0.86602540378444004), s2, s5);
  const double s9 = s2*s2;
  const double s11 = fma((-0.25), s9, (1));
  const double s13 = fma(s4, s6, s11);
  const double s14 = f[2];
  const double s15 = s14*s14;
  const double s16 = s13*s15;
  return s16;
}
}} // namespace numtracer_kernels::cplxdisc_num
