...
#include "Epsadj_num_kernels.hh"

namespace numtracer_kernels
{
  template<typename REG>
  class Epsadj_num_kernel
  {
    public:
    static inline auto kernel(const double& l1, const double& cos1, const double& cos2, const double& p)
    {
      ...
      double fenv[(numtracer_kernels::epsadj_num::nenv) > 0 ? (numtracer_kernels::epsadj_num::nenv) : 1];
      numtracer_kernels::epsadj_num::fill(fenv, l1, cos1, cos2, p);  return numtracer_kernels::epsadj_num::tr0(fenv);
    }
...
using numtracer_kernels::Epsadj_num_kernel;