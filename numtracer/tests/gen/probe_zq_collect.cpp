...
static inline auto probe_proj(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  const auto _interp10 = ntStub(29441., 1. * pow(1. + powr<6>(k),0.16666666666666666667));
  const auto _interp11 = ntStub(89189., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _den1 = powr<-1>(_interp10 * _interp11 + _interp9 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _den2 = powr<-2>(powr<2>(_interp7) + powr<2>(_interp3 * _interp4 + _interp8 * l1));
  const auto _cse1 = powr<-1>(l1);
  return -0.0833333333333333 * _den1 * _den2 * DiFfRG::zq_collect::tr0(fenv) * powr<2>(_interp1) * powr<-2>(p) * fma(-1., _cse1 * _interp2 * _interp3, fma(-1., _cse1 * _interp4 * (_interp5 + 50. * (-_interp3 + _interp6)), 0.));
}
