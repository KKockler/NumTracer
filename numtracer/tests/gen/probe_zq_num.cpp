...
static inline auto probe_proj(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  const auto _cse1 = powr<-1>(l1);
  return -0.0833333333333333 * _den1 * _den2 * DiFfRG::zq_num::tr0(fenv) * powr<2>(_interp1) * powr<-2>(p) * fma(-1., _cse1 * _interp2 * _interp3, fma(-1., _cse1 * _interp4 * (_interp5 + 50. * (-_interp3 + _interp6)), 0.));
}
